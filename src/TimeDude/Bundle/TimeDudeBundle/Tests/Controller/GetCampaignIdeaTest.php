...
class GetCampaignIdeaTest extends WebTestCase {
    ...
    public function testGetCampaignIdeaStatus200() {

        $client = static::createClient();
        $crawler = $client->request(
                'GET', 'api/v1/campaigns/' . $this->existing_campaign_id . '/idea', array(), array(), array(
        ...
        $this->assertContains('campaign_idea', $response);
    }

    public function testGetCampaignIdeaNoAuthentication500() {
        $client = static::createClient();
        $crawler = $client->request(
                'GET', 'api/v1/campaigns/' . $this->existing_campaign_id . '/idea', array(), array(), array()
        );
    ...
    public function testGetCampaignIdeaInvalidAuthentication403() {
        $client = static::createClient();
    ...
    public function testGetCampaignIdeaCampaignNotFound404() {
        $client = static::createClient();

        $crawler = $client->request(
                'GET', 'api/v1/campaigns/' . $this->invalid_campaign_id . '/idea', array(), array(), array(
            'HTTP_x-wsse' => 'ApiKey="' . $this->existing_api_key . '"',
                )
        );
